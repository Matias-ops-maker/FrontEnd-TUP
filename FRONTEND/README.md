...
## � Ejecutar cuando FRONT y BACK estén en repositorios separados

Este proyecto está pensado para publicarse en dos repositorios distintos: uno para el `FRONTEND` y otro para el `BACKEND`.

Pasos rápidos para levantar ambos en tu máquina (PowerShell):

1) Clonar ambos repositorios en carpetas separadas (por ejemplo `repuestos-frontend` y `repuestos-backend`).

2) Backend: instalar dependencias y ejecutar
```powershell
cd repuestos-backend
npm install
# (opcional) migrar y poblar la BD
npm run migrate
npm run seed
# iniciar servidor
npm start
...
3) Frontend: configurar la URL del backend y ejecutar
```powershell
cd repuestos-frontend
# Crear .env con la URL del backend (ejemplo)
Set-Content -Path .env -Value "VITE_API_URL=http://localhost:4000/api`nVITE_MP_PUBLIC_KEY=tu_public_key_de_mercadopago"
npm install
npm run dev
```

4) Verificación: abre `http://localhost:5173` y prueba el login con las credenciales de prueba (admin/admin123 o juan@gmail.com/user123 si seed los creó).

Notas importantes:
- Asegúrate que el backend permita CORS desde la URL del frontend (por ejemplo `http://localhost:5173`). Ver ejemplo en el README del backend.
- Para producción, configura `VITE_API_URL` apuntando a la URL pública del backend (por ejemplo `https://api.miapp.com/api`).
- No olvides mantener secretas las claves (no subir `.env` al repositorio). Incluye un `.env.example` en cada repo con variables necesarias.

Si quieres, puedo añadir ejemplos de `.env.example` y comandos `git` para inicializar cada repo. Solo dime si los quieres aquí mismo.
